...
def main(source_file, output_file, strategy, source_cmap, source_level, cmap, level):
    ...
    if strategy == 'intensity':
        ...
        values = 255 - np.array(source).sum(2) // 3
    elif strategy == 'nearest':
        # match by finding the neareast centroids in a source colormap
        dists = (np.abs(source[None] - matchpal[:, None, None])).sum(3)
    ...
    img = Image.fromarray(values.astype(np.uint8), mode='P')
